...
data <- data.frame(read.table("data/pre_cleveland.txt", col.names = colNames, header=FALSE, sep =" "))
...
data <- data.frame(read.table("data/cleveland_data.txt", col.names = colNames, header=FALSE, sep =" "))
cleveland.lm <- lm(num ~ age + sex + cp + trestbps + chol + fbs + restecg + thalach + exang + oldpeak + slope + ca + thal, data=data)
...
data <- data.frame(read.table("data/pre_combined.txt", col.names = colNames, header=FALSE, sep =" "))
...
data <- data.frame(read.table("data/combined_data.txt", col.names = colNames, header=FALSE, sep =" "))
combined <- lm(num ~ age + sex + cp + restecg + thalach + exang + set_id, data=data)
plot(combined, which=5, id.n = 20)
abline(2,0, lty=4, col = 59 )
abline(-2,0, lty=4, col = 59 )
